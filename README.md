## 👋 Olá, eu sou a Beatriz Cavalcante

💻 Desenvolvedora em formação  
🚀 Apaixonada por tecnologia e aprendizado contínuo  

---

### 🧠 Sobre mim
- 🎓 Análise e Desenvolvimento de Sistemas — SENAC (Embarque Digital)
- 💼 Experiência com suporte ao usuário
- 📊 Atuação com análise de dados utilizando Power BI e SQL
- 🌐 Desenvolvimento web em constante evolução
- 🐍 Estudando desenvolvimento de APIs com Python
- 🐳 Aprendendo Docker para criação e gerenciamento de ambientes

---

### 📚 Formação complementar
- Com Domínio Digital — Instituto Aliança  
- Desenvolvimento Angular — DIO + Banco Pan  
- DecolaTech — DIO + Avanade  
- Desenvolvimento Web — IOS  

---

### 🚀 Atualmente
- 💻 Trabalhando com suporte ao usuário
- 📊 Criando dashboards e análises com Power BI
- 🗄️ Utilizando SQL para manipulação e consulta de dados
- 🌐 Desenvolvendo aplicações web
- 🐍 Criando APIs com Python (Flask)
- 🐳 Containerizando aplicações com Docker

---

### 📫 Contato
- 💼 [LinkedIn](https://www.linkedin.com/in/beatriz-cavalcante-554542271/)
- 📷 [Instagram](https://www.instagram.com/beatriiz_cavalcante)

---

### 📊 Estatísticas
<div align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Beatriiz-Cavalcante&layout=compact&theme=dracula"/>
</div>

---

### 🛠️ Tecnologias & Ferramentas
<div>
  <img src="https://skillicons.dev/icons?i=html,css,js,angular,python,flask,docker,git,java" />
</div>